...
export const World = () => {
    ...
    const CallData = async () => {
        let promise = await fetch("https://newsapi.org/v2/everything?q=USA&apiKey=5bac53e30f2c401a869eee92662216e8");
    ...
    return (
        <MainNewsContainerStyle>
            <div style={{ margin: "20px 10px 10px", fontSize: "16px", fontWeight: "500", color: "#424242" }}><span style={{ color: "#00b1cd" }}>Home</span> / World</div>
            <div style={{ display: "flex", alignItems: "center", marginBottom: "20px" }}>
                ...
                <div style={{ fontSize: "20px", fontWeight: "bold", textAlign: "center", width: "50%" }}><span className='bracketHome'>[</span>WORLD NEWS<span className='bracketHome'>]</span></div>
            ...
            {data.map((el, i) => {
                if (el.description && el.urlToImage) {
                    if (el.description[0] !== "<") {
                        ...
                        if (count === 1) {
                            return (
                                <FirstNewsComponent el={el} title={title} month={month} date={date} year={year} hour={hour} min={min} zone={zone} heading={"WORLD NEWS"} id={1100 + i} />
                            )
                        }
                        else {
                            return (
                                <OtherNewsComponents el={el} title={title} month={month} date={date} year={year} hour={hour} min={min} zone={zone} heading={"WORLD NEWS"} id={1100 + i} />
                            )
                        }
