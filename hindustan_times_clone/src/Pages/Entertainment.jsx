...
export const Entertainment = () => {
    ...
    const CallData = async () => {
        let promise = await fetch("https://newsapi.org/v2/everything?q=Entertainment&apiKey=5bac53e30f2c401a869eee92662216e8");
            ...
            {data.map((el, i) => {
                if (el.description && el.urlToImage) {
                    if (el.description[0] !== "<") {
                        ...
                        if (count === 1) {
                            return (
                                <FirstNewsComponent el={el} title={title} month={month} date={date} year={year} hour={hour} min={min} zone={zone} heading={"ENTERTAINMENT"} id={500 + i} />
                            )
                        }
                        else {
                            return (
                                <OtherNewsComponents el={el} title={title} month={month} date={date} year={year} hour={hour} min={min} zone={zone} heading={"ENTERTAINMENT"} id={500 + i} />
                            )
                        }
